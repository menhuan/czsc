...
from czsc.data.bian_cache import BiAnDataCache
from czsc.enum import BiFreq
...
bars = dc.bian_btc_daily(ts_code='DYDXUSDT', interval=BiFreq.F5.value)
print("输出结果",bars)
c = CZSC(bars)
c.open_in_browser()
...
bg = BarGenerator(base_freq=Freq.F5.value, freqs=[Freq.F30.value,  Freq.F4H.value], max_count=5000)
for bar in bars:
    bg.update(bar)
